...
const allUpperCase = (arr) => {
  let marionette = [];
  arr.forEach(element => {
    marionette.push(element.toUpperCase());
  });
  return marionette;
};


// get it ? because strings?
...
function speaker(arr,greeting){
  let newArr = arr.map(element=>greeting(element));
  return newArr;
}
