...
const screenForNames = (arr) => {
  let regexStart = /(^Mr. [a-zA-Z])|(^Mrs. [a-zA-Z])|(^Ms. [a-zA-Z])|(^Dr. [a-zA-Z])/;
  let regexNumbers = /\d/;
  return arr.filter((element) => {
    return (element.match(regexStart) && !element.match(regexNumbers));
  });
...
const toTitleCase = (arr) => {
  ...
  return arr.map((element) => {
    newString = '';
    newString += element.charAt(0).toUpperCase();
    newString += element.slice(1, element.length);
    return newString;
  });
};
...
let biggerThanLuke = (arr) => {
  ...
  let newArray = arr.filter((element) => +element.mass > +LukeWeight);
  newArray = newArray.map((element) => element.name);
