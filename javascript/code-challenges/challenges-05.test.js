...
const toLastNames = (people) => people.map(people=> combineNames(people.firstName,people.lastName));
...
const addPurchases = (arr)=> arr.reduce((total,price)=>{
  return price.purchasePrice+total;
}, 0);
...
const countNumberOfElements = (arr) => arr.reduce((total,elements)=>{
  return elements;
},0);
...
const returnNames = (arr) => arr.reduce((total,names,idx)=> {
  total.push(names.name);
  return total;
},[]);
...
let reversedString = () => arr.reduce((newStr,char,idx)=> {
